...
const multiDest = require("gulp-multi-dest"); // Pipes to multiple destinations.
const through2 = require("through2"); // Update the last modified time. @see https://stackoverflow.com/a/52303073
...
gulp.task("styles", () => {
  return gulp
    ...
    .pipe(
      // Function to update the last modified time of our css files via through2.
      // @see https://stackoverflow.com/a/52303073
    ...
    .pipe(
      multiDest([
        config.styleWordpressDestination,
        config.styleShopifyDestination
      ])
    )
...
gulp.task("stylesRTL", () => {
  return gulp
    ...
    .pipe(gulp.dest(config.styleWordpressDestination))
    .pipe(gulp.dest(config.styleShopifyDestination))
    .pipe(filter("**/*.css")) // Filtering stream to only css files.
