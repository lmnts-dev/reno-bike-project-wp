...
abstract class Jet_Woo_Product_Gallery_Base extends Widget_Base {
	...
	protected function register_base_video_controls( $css_scheme ) {
		...
		$this->add_control(
			'self_hosted_min_height',
			array(
				'label'       => esc_html__( 'Video Height', 'jet-woo-product-gallery' ),
				'description' => esc_html__( 'Worked just with self hosted video types', 'jet-woo-product-gallery' ),
				'type'        => Controls_Manager::NUMBER,
			)
		);
	...
	public function __get_video_html() {
		...
		if ( 'self_hosted' === $video_type ) {
			...
			if ( filter_var( $settings['self_hosted_min_height'], FILTER_VALIDATE_BOOLEAN ) ) {
				$this->add_render_attribute( 'video_player', 'height', $settings['self_hosted_min_height'] );
			}
